...
def test_filter_off_ac():
    ...
    ### 3) neither 
    data = {"geds":{"cut":[[True,True],[False],[False,False]],
    ...
    assert ak.all(data_filtered["geds","cut"]==ak.Array([[True,True],[True],[True]]))
    assert ak.all(data_filtered["geds","is_good_channel"]==ak.Array([[True,True],[True],[True]]))
    assert ak.all(data_filtered["geds","hit_rawid"]==ak.Array([[1,2],[1],[2]]))
    assert ak.all(data_filtered["geds","energy"]==ak.Array([[100,200],[500],[100]]))
    assert ak.all(data_filtered["geds","unphysical_hit_rawid"]==ak.Array([[],[],[]]))

    ### filter an AC this should just change the cut

    data = {"geds":{"cut":[[True,True],[True],[True,False]],
    ...
    assert ak.all(data_filtered["geds","cut"]==ak.Array([[True,True],[False],[True,False]]))
    assert ak.all(data_filtered["geds","is_good_channel"]==ak.Array([[True,True],[False],[True,False]]))
