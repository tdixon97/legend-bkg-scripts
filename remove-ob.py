...
def get_quantiles(counts, bins):
    ...
    quantile_bins = [
        np.argmax(cumulative_counts >= q / 100 * total_count)
...
def run_example(N1, frac, MT2, MT1=44, label="Tl", make_plots=True, pdf=None):
    ...
    if make_plots:
        ...
        axes.set_xlabel(r"$\mu_{other}$ [cts/day]")
        axes.set_ylabel(r"$\mu_{OB}$ [cts/day]")
        ...
        axes.set_title(
            f"{100-100*frac:.0f}% reduction, {MT2} kg-yr, {label}", fontsize=8
        )
        ...
        axes.set_xlabel(r"$\mu$ [cts/kg-yr]")
        axes.set_ylabel("Prob [arb units]")

        axes.axvline(x=mu1, color="b", linestyle="--", label="Current rate")
        histo_x.plot(
            ax=axes, **style, histtype="fill", alpha=0.5, label="Other sources"
        )
    ...
    for type_analysis in ["Tl", "Bi"]:
        ...
        with PdfPages(f"plots/remove_ob_{type_analysis}_gamma.pdf") as pdf:
            ...
            for col, frac in zip(colors, [0.1, 0.3, 0.7, 0.9]):
                ...
                axes.set_xlabel("Exposure [kg-yr]")
                axes.set_ylabel(r"$\mu_{ob}$ [kg-yr]")
            ...
            for col, frac in zip(colors, [0.1, 0.3, 0.7, 0.9]):
                ...
                axes.plot(exp, med, color=col)
                axes.set_xlabel("Exposure [kg-yr]")
                axes.set_ylabel(r"$\mu_{ot}$ [kg-yr]")
